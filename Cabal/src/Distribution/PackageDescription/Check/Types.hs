...
data CheckInterface m =
        CheckInterface { ciPureChecks :: Bool,
                            -- Perform pure checks?
...
pristineCheckCtx :: Monad m => CheckInterface m -> GenericPackageDescription ->
                    CheckCtx m
...
initPNames gpd = PNames (package . packageDescription $ gpd)
...
--
newtype CheckM m a = CheckM (Reader.ReaderT (CheckCtx m)
                               (Writer.WriterT (Set.Set PackageCheck)
                                 m)
                               a)
...
liftInt :: forall m i. Monad m =>
             ...
             CheckM m ()
liftInt acc f = do ops <- asksCM (acc . ccInterface)
                   maybe (return ()) l ops
    where
          l :: i m -> CheckM m ()
          l wi = do cks <- liftCM (f wi)
...
--
check :: Monad m => Bool ->         -- Is there something to warn about?
...
--
checkP :: Monad m => Bool ->         -- Is there something to warn about?
                     PackageCheck -> -- Warn message.
                     CheckM m ()
...
--
checkIntDep :: forall m i. Monad m =>
                 (CheckInterface m -> Maybe (i m)) ->
                   -- Check interface, may or may not exist. If it does not,
                   -- the check simply will not be performed.
...
checkInt :: forall m i. Monad m =>
              ...
              CheckM m ()
checkInt acc f ck = checkIntDep acc (\ops -> do b <- f ops
                                                if b
                                                  then return $ Just ck
                                                  else return Nothing)
...
ppExplanation (InvalidNameWin pkg) =
    "The package name '" ++ prettyShow pkg ++ "' is "
...
ppExplanation (LicenseMessParse lic) =
    "Unfortunately the license " ++ quote (prettyShow lic)
...
ppExplanation (COptONumber prefix label) =
    "'" ++ prefix ++": -O[n]' is generally not needed. When building with "
      ...
      ppWarnLang label ++ " code. Setting it yourself interferes with the"
...
ppExplanation (InvalidOnWin paths) =
  ...
  where quotes [failed] = "path " ++ quote failed ++ " is"
        quotes failed = "paths " ++ commaSep (map quote failed)
                          ++ " are"
